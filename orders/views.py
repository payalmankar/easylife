...
def get_counts_as_date(request):
    if request.user.is_authenticated:
        ...
        try:
            if from_date == 'None':
                from_date = timezone.datetime(1900, 1, 1, tzinfo=timezone.utc)
            if to_date == 'None':
            ...
            all_store = Store.objects.filter(date_created__range=(from_date, to_date)).count()

            return JsonResponse({
                'orders_count': all_order,
                'revenue_count' : total_revenue,
                'all_store' : all_store
                })
        except Exception as e:
            print("Exception occired")
